...
QrcodeWidget::ScanQrcodeTask::ScanQrcodeTask(QObject *parent)
    : QThread(parent) {
  start();
}
...
QString QrcodeWidget::get_qrcode(cv::Mat &img) {
  ...
  cv::adaptiveThreshold(img, adaptive_img, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C,
  ...
  while (result.empty() && thre < 110) {
    ...
    if (!result.empty()) printf("thr=%d result=%s\n", thre, result.c_str());
...
void QrcodeWidget::scan_qrcode_ui() {
  ...
  pos_y_ += 10;
  if (pos_y_ > max_pos_y) pos_y_ = 100 + 2;
  pl_lightneedle_->move(pos_x_, pos_y_);
}
...
void QrcodeWidget::scan_qrcode() {
  ...
  if (ret == SZ_RETCODE_OK) {
    ...
    /*
                    ...
                    if (!ive->yuv2RgbPlannar(mmz_bgr_image, mmz_yuv_image,
       true)) return;*/

    cv::Mat yuvImage(mmz_yuv_image->height, mmz_yuv_image->width, CV_8UC1,
                     mmz_yuv_image->pData);
    // cv::flip(yuvImage, yuvImage, 1);
    // cv::Mat input_image(mmz_yuv_image->height, mmz_yuv_image->width,
    // CV_8UC3); cvtColor(yuvImage, input_image, CV_YUV2BGR_NV21);

    // cv::Mat input_image(mmz_bgr_image->height, mmz_bgr_image->width, CV_8UC3,
    // (unsigned char *)mmz_bgr_image->pData);
    cv::Mat qr_image = yuvImage(
        cv::Rect((screen_width_ - QRCODE_WIDTH) * 1.34 / 2,
                 /*(screen_height_ - QRCODE_HEIGHT)* 1.35 / 2*/ 100 * 1.35,
                 QRCODE_WIDTH * 1.34, QRCODE_HEIGHT * 1.35));

    cv::flip(qr_image, qr_image, 1);
    cv::rectangle(
        yuvImage,
        cv::Rect((screen_width_ - QRCODE_WIDTH) * 1.34 / 2, 100 * 1.35,
                 QRCODE_WIDTH * 1.34, QRCODE_HEIGHT * 1.35),
        cv::Scalar(0, 0, 255));
    static int index = 0;
    ...
    index++;
		...
		//if (!str.isEmpty())
			qDebug() << str;
#else
    QString result = get_qrcode(qr_image);
    if (!result.isEmpty()) {
      static PersonData person_data;
      if (SZ_RETCODE_OK == PersonService::get_instance()->get_person(
                               result.toStdString(), person_data)) {
