...
TemperatureTipWidget::TemperatureTipWidget(int screen_width, int screen_height, QWidget *parent)
    : QWidget(parent) {

	setAttribute(Qt::WA_StyledBackground, true);
	setStyleSheet("QWidget {background-color:rgba(5, 0, 20, 150);margin:0px;border-radius:10px;} QLabel {background-color:transparent;color:white;}");
	...
	pl_temperature_->setStyleSheet("QLabel {background-color:transparent;color:white;}");
...
void TemperatureTipWidget::rx_temperature(bool bvisible, bool bnormal_temperature, float temperature) {
	if (!bvisible) {
		hide();
		return;
	}
	...
	pl_temperature_->setText(tr("体温") + ":" + QString::number(temperature, 'f', 1) + "°C");


	//
	style_str = "QWidget {background-color:rgba(220, 0, 0, 150);margin:0px;border-radius:10px;} QLabel {background-color:transparent;}";
	if (bnormal_temperature)
		style_str = "QWidget {background-color:rgba(0, 100, 0, 180);margin:0px;border-radius:10px;} QLabel {background-color:transparent;}";
