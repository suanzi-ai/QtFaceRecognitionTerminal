...
void FaceTimer::screen_saver_timeout() { display_screen_saver(true); }
...
void FaceTimer::white_led_timeout() {
  ...
  if (MAX_WHITE_LED_TIMEOUT >= screensaver_timeout)
    ...
    screen_saver_timer_->start((screensaver_timeout - MAX_WHITE_LED_TIMEOUT) *
                               1000);
  }
}

void FaceTimer::delay_close_white_led() {
  if (bclose_white_led_) LEDTask::get_instance()->turn_off();
}
